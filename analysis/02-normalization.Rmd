...
Create filter for lowly expressed and detected genes:
...
plotRLE(sce[!nan, ], colour_by = "stage")
```
...
plt <- lapply(var, function(x) plotPCA(sce, colour_by = x, shape_by = "stage"))

wrap_plots(plt, ncol = 2) & coord_fixed(1)
```
...
wrap_plots(plt, ncol = 2) & theme(aspect.ratio = 1)
```
