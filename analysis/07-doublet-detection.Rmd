...
knitr::opts_chunk$set(
  autodep    = TRUE,
  cache      = TRUE,
  cache.path = "cache/07-doublet-detection.Rmd/",
  dev        = "png",
  error      = FALSE,
  message    = FALSE,
  warning    = FALSE
...
```{r}
sce <- readRDS("output/06-clustering.rds")
```
...
```{r}
plotReducedDim(sce, dimred = "corrected", colour_by = "doublet_score", shape_by = "stage")
```
...
```{r}
saveRDS(sce, file = "output/07-doublet-detection.rds" )
```
