...
```{r}
file.copy("output/09-cell-annotation.rds", "shiny/data.rds", overwrite = TRUE)
```
...
# Create iSEE object
iSEE(sce, appTitle = "e-NMPs CLE (E8.5/E9.5) [Unintegrated]", colormap = ecm)
```

Deploy the iSEE application to the shiny server:

```{r}
deployApp(appDir = "shiny", appName = "GOUTI_2017_NMP_PCA", appTitle = "e-NMPs CLE (E8.5/E9.5) [Unintegrated]", launch.browser = FALSE, forceUpdate = TRUE)
```

Allocate more memory to the iSEE application:

```{r}
configureApp(appName = "GOUTI_2017_NMP_PCA", appDir = "shiny", size = "xlarge")
```
