...
```{r}
ids <- strsplit(
  x = rowData(sce)$gene_id,
  split = ".",
  fixed = TRUE
)

ids <- sapply(ids, .subset, 1)

rowData(sce)$gene_id <- ids
...
```{r}
colnames(sce) <- uniquifyFeatureNames(colData(sce)$cell, colData(sce)$sample)
```
...
mit <- which(rowData(sce)$gene_id %in% ids)
```
...
dat <- DataFrame(
  ...
  high_altexps_spike_percent = sce$altexps_spike_percent > 25
)
