...
knitr::opts_chunk$set(
  autodep    = TRUE,
  cache      = TRUE,
  cache.path = "cache/06-clustering.Rmd/",
  dev        = "png",
  error      = FALSE,
  message    = FALSE,
  warning    = FALSE
...
```{r}
sce <- readRDS("output/05-reduced-dimensions.rds")
```
...
```{r}
var <- c("cluster_walktrap", "sum", "detected", "altexps_spike_percent")
```

#### PCA

```{r, fig.height = 8, fig.width = 10}
plt <- lapply(var, function(x) plotReducedDim(sce, dimred = "corrected", colour_by = x, shape_by = "stage") + coord_fixed())
...
```{r, fig.height = 8, fig.width = 10}
plt <- lapply(var, function(x) plotTSNE(sce, colour_by = x, shape_by = "stage") + theme(aspect.ratio = 1))
...
```{r, fig.height = 8, fig.width = 10}
plt <- lapply(var, function(x) plotUMAP(sce, colour_by = x, shape_by = "stage") + theme(aspect.ratio = 1))
...
```{r}
var <- c("cluster_louvain", "sum", "detected", "altexps_spike_percent")
```
...
```{r}
var <- c("cluster_kmeans", "sum", "detected", "altexps_spike_percent")
```
...
```{r}
var <- c("cluster_hclust", "sum", "detected", "altexps_spike_percent")
```
...
```{r}
saveRDS(sce, file = "output/06-clustering.rds")
```
