...
```{r}
res <- findMarkers(sce, groups = sce$cluster, test.type = "t", pval.type = "any")
...
for (n in names(sig)) {
  ...
  plt <- lapply(ids, function(x) {
    ...
    plotUMAP(sce, colour_by = x, shape_by = "stage", by_exprs_values = "logcounts", text_by = "cluster") + 
      ggtitle(x) + 
...
```{r}
res <- findMarkers(sce, groups = sce$cluster, test.type = "t", pval.type = "all")
...
```{r}
res <- findMarkers(sce, groups = sce$cluster, test.type = "wilcox", pval.type = "any")
...
```{r}
res <- findMarkers(sce, groups = sce$cluster, test.type = "wilcox", pval.type = "all")
...
```{r}
res <- findMarkers(sce, groups = sce$cluster, test.type = "binom", pval.type = "any")
...
```{r}
res <- findMarkers(sce, groups = sce$cluster, test.type = "binom", pval.type = "all")
